...
<div class="attendance-list">
    ...
    <div class="month-select">
        ...
        <a href="{{route('admin.attendance.staff', ['month'=>$prevMonth, 'id'=>$user->id])}}">
        ...
        <a href="{{route('admin.attendance.staff', ['month'=>$nextMonth, 'id'=>$user->id])}}">
            <p>翌 月</p>
...
<table class="attend-table">
    ...
    <tbody>
        ...
        @if ($attendance && $attendance->clock_in_time)
            ...
            <td>{{ optional(optional($attendance)->clock_out_time)->format('H:i') }}</td>
        ...
        @endif
            <td>
                @if ($attendance && $attendance->calcBreakMinutes() > 0)
                    {{ gmdate('H:i', $attendance->calcBreakMinutes() * 60) }}
            ...
            <td>
                @if ($attendance && $attendance->calcWorkMinutes() > 0)
                    {{ gmdate('H:i', $attendance->calcWorkMinutes() * 60) }}
            ...
            <td>
                ...
                @else
                <a href="{{ route('admin.attendance.detail', ['id' =>  0, 'work_date' => \Carbon\Carbon::parse($day['date'])->toDateString(),'user_id'=>$user->id]) }}" class="detail-link">詳細</a>
                @endif  
            </td>
        </tr>
        @endforeach
    ...
    <div class="csv-output">
        <a href="{{ route('admin.attendance.csv', ['user_id' => $user->id, 'month' => $thismonth]) }}" class="csv-output-button">
        CSV出力</a>
    </div>
</div>
@endsection