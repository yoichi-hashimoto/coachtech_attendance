...
@elseif($attendance->id <=0))
<form action="{{ route('admin.stamp_correction_request.store') }}" method="POST">
    @csrf
...
@else
<form action="{{ route('admin.stamp_correction_request.store', ['id' => optional($attendance)->id ?? 0]) }}" method="POST">
    @csrf
    <input type="hidden" name="work_date" value="{{ $workDateObj->toDateString() }}">
